...
- Fixed drone registration against the live schema by generating `drones.drone_id` explicitly in `app/admin/actions.ts` instead of assuming the database auto-populates it. At the same time, removed charging from the admin fleet/overview/batch UI, from the documented drone status model, and from the checked-in schema reference/migration files so the project stays aligned with the presentation scope.
- Fixed the fleet registration form so it no longer hardcodes `DX-100`. `lib/admin-ops.ts` now exposes the live `models` table to the admin UI, `app/admin/fleet/page.tsx` renders those real model options, and `app/admin/actions.ts` now verifies the submitted `modelId` exists before inserting a drone. Also verified against live Supabase that multiple models are present (`DX-100`, `SwiftDrop S2`, `CargoLift C1`, `SkyReach X4`, `MicroHop M8`).
- Fixed the remaining drone-registration mismatch in the admin fleet form. The UI no longer asks for a separate operating zone when creating a drone, and `app/admin/actions.ts` now derives `drones.zone_id` directly from the selected hub so hub/zone pairs cannot drift. While checking the live data, also confirmed one older row is still inconsistent (`drone_id = 4` has `home_hub_id = 4` but `current_hub_id = 3`), which explains the misleading zone-4 stationing seen in admin screens.
- Added `scripts/seed-demo-orders.mjs` plus `npm run demo:seed-orders` for presentation seeding. The script loads `.env.local`, looks up the live seller/store data in Supabase, guarantees orders are created for `b23391@students.iitmandi.ac.in`, creates a larger TSP-friendly batch in that seller’s zone, adds a small number of cross-zone orders from another populated seller zone, and allocates batch/order ids dynamically so the script can be rerun safely without hardcoded ids. It also supports `--dry-run` and `--mode=ready|collecting` so demo batches can either dispatch immediately with the simulator or sit in an active collection window.
- Cleaned up inconsistent live fleet rows in Supabase after verifying the admin form bug had already been fixed in code. Found two stale drones (`drone_id = 2` and `drone_id = 4`) whose hub/zone data no longer matched, detached them from completed `delivery_batches` rows, deleted the drone records, and rechecked that no inconsistent drones remain in the live database.
- Fixed a second fleet consistency issue after tracing an "idle DX-100 not on hub" report. The root causes were (1) duplicate UI labels that made different drones look like the same `DX-100`, and (2) manual admin state changes that could mark a drone `available` without snapping `last_known_location` back to its hub. `lib/admin-ops.ts` now labels drones uniquely with their ID, and `app/admin/actions.ts` now resets idle drones to their hub coordinates when a batch is released or manually marked completed/failed.
- Properly fixed the zone-move hub drift after verifying the live Supabase data. The real bug was that `updateZoneGeometry()` updated `hub_location_zone.hub_location` but left `hubs.hub_location` stale, even though drone creation and manual docking still read from `hubs`; live mismatches had grown to roughly 273m-4.7km depending on the zone. `app/admin/actions.ts` now reads hub coordinates authoritatively from `hub_location_zone`, syncs `hubs.hub_location` when a zone hub is moved, and snaps all available drones stationed at that hub to the new point during the same save.
